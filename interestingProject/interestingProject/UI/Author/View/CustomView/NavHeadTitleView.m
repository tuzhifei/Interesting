...
-(instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        self.back.frame = CGRectMake(5, 30 + kSafeAreaTop, 20, 20);
        [self.back addTarget:self action:@selector(backClick) forControlEvents:UIControlEventTouchUpInside];
        [self addSubview:self.back];
        
        self.backgroundColor = [UIColor clearColor];
        self.label = [[UILabel alloc]initWithFrame:CGRectMake(40, 30  + kSafeAreaTop, frame.size.width-80, 30)];
        ...
        self.rightBtn.frame = CGRectMake(self.frame.size.width-45, 30 + kSafeAreaTop, 20, 20);
